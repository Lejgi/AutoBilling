...
local function fetchESX()
    ...
    -- Zkus moderní inicializaci ESX (v2.0+)
    if GetResourceState('es_extended') == 'started' then
        ...
        if ok and obj then
            ESX = obj
            print('[RecurringBilling] ESX loaded using getSharedObject() export.')
            return ESX
        end

        -- Pokud moderní metoda selže, zkus globální proměnnou (některé buildy ji exportují automaticky)
        if _G.ESX then
            ESX = _G.ESX
            print('[RecurringBilling] ESX loaded via global variable.')
            return ESX
        end
    end
    ...
    if legacyObject then
        ESX = legacyObject
        print('[RecurringBilling] ESX loaded via legacy getSharedObject event.')
        return ESX
    end

    print('^1[RecurringBilling]^0 Nelze načíst ESX object. Zkontroluj, že es_extended běží dřív a resource se jmenuje přesně "es_extended".')
    return nil
end
...
local function sendInvoice(invoice)
    ...
    local success, err = pcall(function()
        ...
        if not targetSource then
            print(('^3[RecurringBilling]^0 Hráč %s není online, faktura bude přeskočena.'):format(invoice.identifier))
            return
        end
        ...
        TriggerServerEvent("okokBilling:CreateCustomInvoice",
            ...
            invoice.amount,
            invoice.label,
            sender, -- invoiceSource
            sender, -- society
            sender:upper(), -- societyName
            "SYSTEM" -- authorIdentifier
        )
    end)

    if not success then
        print(('^1[RecurringBilling]^0 Nepodařilo se vytvořit fakturu pro %s (%s): %s')
            :format(invoice.label, invoice.identifier, err or 'Neznámá chyba'))
        return false
    end
